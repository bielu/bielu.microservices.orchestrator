...
public class ContainerdImageManager : IImageManager
{
    ...
    {
        _logger.LogDebug("Listing containerd images in namespace {Namespace}", _options.Namespace);
    ...
    {
        _logger.LogDebug("Getting containerd image {ImageId} in namespace {Namespace}", imageId, _options.Namespace);
    ...
    {
        _logger.LogInformation("Pulling image {Image}:{Tag} via containerd in namespace {Namespace}", request.Image, request.Tag, _options.Namespace);
    ...
    {
        _logger.LogInformation("Removing containerd image {ImageId}", imageId);
    ...
    {
        _logger.LogInformation("Tagging containerd image {ImageId} as {Repository}:{Tag}", imageId, repository, tag);
