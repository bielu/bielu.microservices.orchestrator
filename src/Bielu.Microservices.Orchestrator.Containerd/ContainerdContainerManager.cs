...
public class ContainerdContainerManager : IContainerManager
{
    ...
    public Task<IReadOnlyList<ContainerInfo>> ListAsync(bool all = false, CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogDebug("Listing containerd containers in namespace {Namespace}", LogSanitizer.Sanitize(_options.Namespace));
    ...
    {
        _logger.LogDebug("Getting containerd container {ContainerId} in namespace {Namespace}", LogSanitizer.Sanitize(containerId), LogSanitizer.Sanitize(_options.Namespace));
    ...
    {
        _logger.LogInformation("Creating containerd container from image {Image} in namespace {Namespace}", LogSanitizer.Sanitize(request.Image), LogSanitizer.Sanitize(_options.Namespace));
    ...
    {
        _logger.LogInformation("Starting containerd task for container {ContainerId}", LogSanitizer.Sanitize(containerId));
    ...
    {
        _logger.LogInformation("Stopping containerd task for container {ContainerId}", LogSanitizer.Sanitize(containerId));
    ...
    {
        _logger.LogInformation("Removing containerd container {ContainerId}", LogSanitizer.Sanitize(containerId));
    ...
    {
        _logger.LogDebug("Getting logs for containerd container {ContainerId}", LogSanitizer.Sanitize(containerId));
